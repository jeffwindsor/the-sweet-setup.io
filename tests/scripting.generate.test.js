...
describe('Scripting', () => {
  describe('Generate', () => {
    ...
    test('Arch Package gives a pacman call', () => run(
      { type: "pacman", package_name: "arch" },"sudo pacman -S --noconfirm arch"));

    test('Yay Package gives a yay call', () => run(
      { type: "Yay", package_name: "aur" },"yay -S --noconfirm aur"));

    test('Brew Package', () => run(
      { type: "Brew", package_name: "brew" },"brew install brew"));

    test('Cask Package', () => run(
      { type: "Cask", package_name: "cask" },"brew cask install cask"));

    test('Npm Package', () => run(
      { type: "Npm", package_name: "npm" },"npm install npm"));

    test('VS code install extension', () => run(
      { type: "code", extension_name: "vsc" },"code --install-extension vsc"));

    test('Haskell Stack Install', () => run({ type: "Stack", package_name: "package" },"stack install package"));
    ...
    test('git config global', () => run(
      { type: "gitconfig", name: "alias ga", value: "git action" },"git config --global alias ga 'git action'"));

    test('Git Clone', () => run(
      { type: "GitClone", uri: "http://url.git" },"git clone http://url.git"));

    test('Git Clone to target', () => run(
      { type: "GitClone", uri: "http://url-target.git", output_dir: "/user/home/two-x" },"git clone http://url-target.git /user/home/two-x"
    ));

    test('Git Clone with args to target ', () => run(
      { type: "GitClone", uri: "http://url-target-args.git", output_dir: "/user/home/two-x", args: "--depth=1" },"git clone http://url-target-args.git /user/home/two-x --depth=1"
    ));
